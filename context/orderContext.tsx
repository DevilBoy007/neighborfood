...
export const OrderProvider = ({ children }: OrderProviderProps) => {
  ...
  const initializeOrders = async (userId: string) => {
    ...
    try {
      ...
      const currentOrdersData = allOrders.filter(
      ...
      console.log(
        `Initialized: ${userPlacedOrders.length} placed orders, ${userReceivedOrders.length} received orders, ${currentOrdersData.length} current orders, ${orderHistoryData.length} historical orders`
      );
    } catch (error) {
  ...
  const refreshOrders = async (userId: string) => {
    ...
    try {
      ...
      console.log(
        `Refreshed: ${userPlacedOrders.length} placed orders, ${userReceivedOrders.length} received orders, ${orderHistoryData.length} historical orders`
      );
    } catch (error) {
  ...
  const updateOrderStatus = async (orderId: string, shopId: string, status: OrderStatus) => {
    try {
      ...
      if (selectedOrder && selectedOrder.id === orderId && selectedOrder.shopId === shopId) {
        setSelectedOrderState((prevOrder) => (prevOrder ? { ...prevOrder, status } : null));
      }
