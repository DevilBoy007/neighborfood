export const useOrderStatus = () => {
    ...
    const getStatusOptions = (status: string, shopOwnerView: boolean = false) => {
        ...
        if (shopOwnerView) {
            ...
            switch (status) {
                ...
                case 'ready':
                    return [
                        { label: 'Contact', targetStatus: 'ready', color: '#00bfff' }
                    ];
                case 'in-delivery':
                    return [
                        { label: 'Contact', targetStatus: 'in-delivery', color: 'orchid' }
                    ];
                case 'completed':
                    return [
    ...
    const buildStatusButtons = (status: string, shopOwnerView: boolean, onStatusChange?: (newStatus: string) => void) => {
